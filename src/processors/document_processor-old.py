...
class DocumentProcessor:
    ...
    def __init__(self, input_file_path, input_bucket, doc_category, doc_language, output_language, input_filename, bedrock_project_arn=None):
        """
        ...
        self.original_filename = input_filename
        ...
        try:
            ...
            try:
                ...
                if self.doc_language != 'english':
                    ...
                    final_results = self.bda_processor.run_bda(self.input_file_path)
                    ...
                    def process_english_page(page_data):
                        ...
                        try:
                            ...
                            return i, table_content, image_content, text_content, page_duration
                        
                        except Exception as e:
                            page_duration = time.time() - page_start_time
                            self.logger.error(f"Error processing English page {i}: {str(e)}")
                            return i, {}, {}, {}, page_duration
                    ...
                    with ThreadPoolExecutor(max_workers=min(2, len(pdf_pages))) as executor:
                        ...
                        for future in as_completed(future_to_page):
                            try:
                                i, table_content, image_content, text_content, page_duration = future.result()
                                page_results[i] = (table_content, image_content, text_content)
                                self.logger.info(f"English page {i} processing completed in {page_duration:.2f}s")
                            except Exception as e:
                                self.logger.error(f"Error processing English page {future_to_page[future]}: {str(e)}")

                    final_results = []
                    # Combine results in page order
                    for i in range(len(pdf_pages)):
                        if i in page_results:
                            table_content, image_content, text_content = page_results[i]
                            combine_text = self.file_processor.combine_text_files(table_content=table_content,
                                                                image_content=image_content, 
                                                                text_content=text_content, 
                    ...
                    # write the merger here
                    

                else:
                    self.logger.info("document category not found")
                import pickle
                with open('final_results_14_3.pkl', 'wb') as f:
                    pickle.dump(final_results, f)
                process_report(combined_all=final_results)
            except Exception as e:
